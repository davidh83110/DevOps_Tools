...
class AwsAclFail2Ban:
    ...
    def ban(self, ip):
	check_ip = self.__check_ip_exist(ip)
    ...
    def __check_ip_exist(self, ip):
        banned_ip_list = self.__get_banned_ip_list()
        check_ip_exist = set(banned_ip_list) & set([ip])

        if check_ip_exist == set([ip]):
            print("ip already be banned")
            return False
        else:
            print("gonna be banned %s" % ip)
            return ip

    def __get_banned_ip_list(self):
        banned_ip_list = []
        for entry in self.network_acl.entries:
            if entry['Egress'] is False:
                banned_ip_list.append(entry['CidrBlock'].split('/')[0])      
    ...
    def __get_available_rule_number(self, ip):
        ...
        if type(rules) is dict:
            ...
            run = True
            while run:
                rule_number = random.randint(1, 800)
